#!/usr/bin/env python3
"""
Comprehensive MESH data processing - extracts all available information from NT format
"""
...
def build_mesh_nt_knowledge_graph(input_file=None, output_dir=None, convert_to_neptune_format=False):
    """
    Build MESH knowledge graph from NT file using BioCypher
    
    Args:
        input_file: Path to the MESH NT file
    ...
    # Check input file
    if not input_file:
        input_file = "mesh2025.nt"
    
    if not os.path.exists(input_file):
        logger.error(f"Input file not found: {input_file}")
        return None
    
    # Initialize BioCypher with comprehensive config
    schema_file = os.path.join("config", "schema_config_comprehensive.yaml")
    bc = BioCypher(
        schema_config_path=schema_file,
        output_directory=output_dir
    )
    
    # Use the comprehensive adapter
    adapter = MeshAdapterComprehensive(input_file)
    
    logger.info(f"Adapter initialization took: {time.time() - start_time:.2f} seconds")
    
    # Get statistics first
    stats = adapter.get_statistics()
    logger.info("=== MESH Data Statistics ===")
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Build MESH Knowledge Graph from NT file")
    parser.add_argument("--input-file", "-i", help="Path to the MESH NT file")
    ...
    try:
        output_dir = build_mesh_nt_knowledge_graph(
        ...
        logger.info("\n" + "=" * 60)
        logger.info("MESH NT Knowledge Graph Built Successfully!")
        logger.info("=" * 60)
        if output_dir:
